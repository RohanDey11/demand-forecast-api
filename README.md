...
- Performance: Prophet 13.17% MAPE, RMSE 69,352.31 (test set, see `scripts/train.py`)
...
│   └── weekly_sales.csv
├── scripts/
│   ├── train.py       # reproduces the Prophet MAPE/RMSE above
│   └── eval.py         # SARIMA baseline comparison
├── tests/
├── Dockerfile
└── requirements.txt
```
...
Reproduce the reported Prophet MAPE/RMSE using the same config as the production service:
